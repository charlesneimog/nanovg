...
struct NVGtextRow {
    ...
    float minx, maxx; // Actual bounds of the row. Logical with and bounds can differ because of
...
// pattern. Solid color is simply defined as a color value, different kinds of paints can be created
...
// Paints
//
// NanoVG supports four types of paints: linear gradient, box gradient, radial gradient and image
// pattern. These can be used as paints for strokes and fills.

// Creates and returns a linear gradient. Parameters (sx,sy)-(ex,ey) specify the start and end
// coordinates of the linear gradient, icol specifies the start color and ocol the end color. The
...
// Creates and returns a box gradient. Box gradient is a feathered rounded rectangle, it is useful
// for rendering drop shadows or highlights for boxes. Parameters (x,y) define the top-left corner
// of the rectangle, (w,h) define the size of the rectangle, r defines the corner radius, and f
// feather. Feather defines how blurry the border of the rectangle is. Parameter icol specifies the
// inner color and ocol the outer color of the gradient. The gradient is transformed by the current
...
// specify the inner and outer radius of the gradient, icol specifies the start color and ocol the
// end color. The gradient is transformed by the current transform when it is passed to
// nvgFillPaint() or nvgStrokePaint().
NVGpaint nvgRadialGradient(NVGcontext *ctx, float cx, float cy, float inr, float outr,
                           NVGcolor icol, NVGcolor ocol);

// Creates and returns an image pattern. Parameters (ox,oy) specify the left-top location of the
// image pattern, (ex,ey) the size of one image, angle rotation around the top-left corner, image is
// handle to the image to render. The gradient is transformed by the current transform when it is
...
NVGpaint nvgDotPattern(NVGcontext *ctx, NVGcolor icol, NVGcolor ocol, float patternSize,
                       float dotRadius, float feather);
...
void nvgDrawRoundedRect(NVGcontext *ctx, float x, float y, float w, float h, NVGcolor icol,
                        NVGcolor ocol, float radius);
//
// Scissoring
//
...
// Adds quadratic bezier segment from last point in the path via a control point to the specified
// point.
...
// Sets the proportional line height of current text style. The line height is specified as multiple
// of font size.
...
// Draws text string at specified location. If end is specified only the sub-string up to the end is
// drawn.
float nvgText(NVGcontext *ctx, float x, float y, const char *string, const char *end);

// Draws multi-line text string at specified location wrapped at the specified width. If end is
// specified only the sub-string up to the end is drawn. White space is stripped at the beginning of
// the rows, the text is split at word boundaries or when new-line characters are encountered. Words
// longer than the max width are slit at nearest character (i.e. no hyphenation).
...
// Calculates the glyph x positions of the specified text. If end is specified only the sub-string
// will be used. Measured values are returned in local coordinate space.
...
int nvgTextBreakLines(NVGcontext *ctx, const char *string, const char *end, float breakRowWidth,
                      NVGtextRow *rows, int maxRows, int skipSpaces);
...
struct NVGparams {
    ...
    void (*renderStroke)(void *uptr, NVGpaint *paint, NVGcompositeOperationState compositeOperation,
                         NVGscissor *scissor, float fringe, float strokeWidth, int lineStyle,
                         float lineLength, const NVGpath *paths, int npaths);
