...
static inline float nvg__sqrtf(float a) {
    return sqrtf(a);
}
static inline float nvg__modf(float a, float b) {
    return fmodf(a, b);
}
static inline float nvg__sinf(float a) {
    return sinf(a);
}
static inline float nvg__cosf(float a) {
    return cosf(a);
}
static inline float nvg__tanf(float a) {
    return tanf(a);
}
static inline float nvg__atan2f(float a, float b) {
    return atan2f(a, b);
}
static inline float nvg__acosf(float a) {
    return acosf(a);
}

static inline int nvg__mini(int a, int b) {
    return a < b ? a : b;
}
static inline int nvg__maxi(int a, int b) {
    return a > b ? a : b;
}
static inline int nvg__clampi(int a, int mn, int mx) {
    return a < mn ? mn : (a > mx ? mx : a);
}
static inline float nvg__minf(float a, float b) {
    return a < b ? a : b;
}
static inline float nvg__maxf(float a, float b) {
    return a > b ? a : b;
}
static inline float nvg__absf(float a) {
    return a >= 0.0f ? a : -a;
}
static inline float nvg__signf(float a) {
    return a >= 0.0f ? 1.0f : -1.0f;
}
static inline float nvg__clampf(float a, float mn, float mx) {
    return a < mn ? mn : (a > mx ? mx : a);
}
static inline float nvg__cross(float dx0, float dy0, float dx1, float dy1) {
...
NVGcontext *nvgCreateInternal(NVGbackend backend) {
    ...
    if (nvg__renderCreate(ctx->backend) == 0) {
        goto error;
    }
    ...
    // Create font texture
    ctx->fontImages[0] =
        nvg__renderCreateTexture(ctx->backend, NVG_TEXTURE_ALPHA, fontParams.width, fontParams.height, 0, NULL);
...
NVGpaint nvgDoubleStroke(NVGcontext *ctx, NVGcolor icol, NVGcolor ocol, NVGcolor dashCol, float dashSize,
                         bool isGradientStroke, bool showActivity, float activityOffset) {
    NVGpaint p;
    NVG_NOTUSED(ctx);
    memset(&p, 0, sizeof(p));
...
void nvgSmoothGlow(NVGcontext *ctx, float x, float y, float w, float h, NVGcolor icol, NVGcolor ocol, float radius,
                   float feather) {
    NVGpaint p;
    memset(&p, 0, sizeof(p));
    nvgTransformIdentity(p.xform);
...
NVGpaint nvgImageAlphaPattern(NVGcontext *ctx, float cx, float cy, float w, float h, float angle, int image,
                              NVGcolor iCol) {
...
void nvgGlobalCompositeBlendFuncSeparate(NVGcontext *ctx, NVGblendFactor srcRGB, NVGblendFactor dstRGB,
                                         NVGblendFactor srcAlpha, NVGblendFactor dstAlpha) {
...
void nvg__tesselateBezierAFD(NVGcontext *ctx, float x1, float y1, float x2, float y2, float x3, float y3, float x4,
                             float y4) {
    // Power basis.
...
static void nvg__flattenPaths(NVGcontext *ctx) {
    ...
    for (int i = 0; i < ctx->ncommands; i++) {
        ...
        case NVG_BEZIERTO:
            ...
            if (last != NULL) {
                nvg__tesselateBezierAFD(ctx, last->x, last->y, values[0], values[1], values[2], values[3], values[4],
                                        values[5]);
            }
            values += 6;
            break;
...
static NVGvertex *nvg_insertSpacer(NVGvertex *dst, NVGpoint *p, float dx, float dy, float w, float u0, float u1,
                                   float t) {
    float dlx = dy;
    float dly = -dx;
    float px = p->x;
...
static NVGvertex *nvg__doubleStrokeCapStart(NVGvertex *dst, NVGpoint *p, float dx, float dy, float w, float d, float aa,
                                            float u0, float u1, float t, float invLength) {
    float px = p->x - dx * d;
    float py = p->y - dy * d;
    float dlx = dy;
    float dly = -dx;
    ...
    nvg__vset(dst, px - dlx * w, py - dly * w, u1, 1, 1, t);
    dst++;
    nvg__vset(dst, px + dlx * w - dx * aa, py + dly * w - dy * aa, u0, 0, -1, t - invLength);
    dst++;
    nvg__vset(dst, px - dlx * w - dx * aa, py - dly * w - dy * aa, u1, 0, 1, t - invLength);
    dst++;

    return dst;
}

static NVGvertex *nvg__doubleStrokeCapEnd(NVGvertex *dst, NVGpoint *p, float dx, float dy, float w, float d, float aa,
                                          float u0, float u1, float t, float invLength) {
    float px = p->x + dx * d;
    float py = p->y + dy * d;
    float dlx = dy;
...
static NVGvertex *nvg__buttCapStart(NVGvertex *dst, NVGpoint *p, float dx, float dy, float w, float d, float aa,
    ...
    float dlx = dy;
    float dly = -dx;
    nvg__vset(dst, px + dlx * w - dx * aa, py + dly * w - dy * aa, u0, 0, -1, t - invLength * aa / w);
    dst++;
    nvg__vset(dst, px - dlx * w - dx * aa, py - dly * w - dy * aa, u1, 0, 1, t - invLength * aa / w);
    dst++;
    nvg__vset(dst, px + dlx * w, py + dly * w, u0, 1, -1, t);
    dst++;
    nvg__vset(dst, px - dlx * w, py - dly * w, u1, 1, 1, t);
...
static NVGvertex *nvg__buttCapEnd(NVGvertex *dst, NVGpoint *p, float dx, float dy, float w, float d, float aa, float u0,
                                  float u1, float t, float invLength) {
    float px = p->x + dx * d;
    float py = p->y + dy * d;
    float dlx = dy;
    float dly = -dx;
...
static NVGvertex *nvg__roundCapStart(NVGvertex *dst, NVGpoint *p, float dx, float dy, float w, int ncap, float aa,
                                     float u0, float u1, float t, int dir) {
    float px = p->x;
    float py = p->y;
    float dlx = dy;
    float dly = -dx;
...
static NVGvertex *nvg__roundCapEnd(NVGvertex *dst, NVGpoint *p, float dx, float dy, float w, int ncap, float aa,
    ...
    float dly = -dx;
    NVG_NOTUSED(aa);
    nvg__vset(dst, px + dlx * w, py + dly * w, u0, 1, w, t);
    dst++;
    nvg__vset(dst, px - dlx * w, py - dly * w, u1, 1, -w, t);
    dst++;
    for (int i = 0; i < ncap; i++) {
        ...
        nvg__vset(dst, px, py, 0.5f, 1, 0, t);
        dst++;
        nvg__vset(dst, px - dlx * ax + dx * ay, py - dly * ax + dy * ay, u0, 1, ax / w, t + invLength * ay / w);
        dst++;
    }
    return dst;
}
...
static int nvg__expandStroke(NVGcontext *ctx, float w, float fringe, int lineCap, int lineJoin, int lineStyle,
                             float miterLimit) {
    NVGpathCache *cache = ctx->cache;
    NVGvertex *verts;
    NVGvertex *dst;
    ...
    for (i = 0; i < cache->npaths; i++) {
        ...
        if (lineStyle > 1 && lineStyle != 5) {
            cverts += 4 * path->count; // extra vertices for spacers
        }
        if (loop == 0) {
            // space for caps
...
void nvgRect(NVGcontext *ctx, float x, float y, float w, float h) {
    ...
    uint8_t commands[] = {NVG_MOVETO, NVG_LINETO, NVG_LINETO, NVG_LINETO, NVG_CLOSE};
...
void nvgRoundedRectVarying(NVGcontext *ctx, float x, float y, float w, float h, float radTopLeft, float radTopRight,
    ...
    if (radTopLeft < 0.1f && radTopRight < 0.1f && radBottomRight < 0.1f && radBottomLeft < 0.1f) {
        ...
        uint8_t commands[] = {NVG_MOVETO, NVG_LINETO,   NVG_BEZIERTO, NVG_LINETO,   NVG_BEZIERTO,
...
void nvgEllipse(NVGcontext *ctx, float cx, float cy, float rx, float ry) {
    ...
    uint8_t commands[] = {NVG_MOVETO, NVG_BEZIERTO, NVG_BEZIERTO, NVG_BEZIERTO, NVG_BEZIERTO, NVG_CLOSE};
    ...
    if (cacheItemIterator != CACHE.end()) {
        ...
        for (int i = 0; i < cacheEntry.paths.size(); i++) {
            ...
            for (int j = 0; j < cachedPath.nstroke; j++) {
                nvgTransformPoint(&cachedPath.stroke[j].x, &cachedPath.stroke[j].y, totalTransform,
                                  cachedPath.stroke[j].x, cachedPath.stroke[j].y);
            }
        }
        ...
        nvg__renderStroke(ctx->backend, &strokePaint, state->compositeOperation, &state->scissor, ctx->fringeWidth,
                          strokeWidth, state->lineStyle, cacheEntry.lineLength, cacheEntry.paths.data(),
                          (int)cacheEntry.paths.size());
        return 1;
    }
    ...
    if (cacheItemIterator != CACHE.end()) {
        ...
        for (int i = 0; i < cacheEntry.paths.size(); i++) {
            ...
            for (int j = 0; j < cachedPath.nfill; j++) {
                nvgTransformPoint(&cachedPath.fill[j].x, &cachedPath.fill[j].y, totalTransform, cachedPath.fill[j].x,
                                  cachedPath.fill[j].y);
            }
        }
        nvgTransformPoint(&cacheEntry.bounds[0], &cacheEntry.bounds[1], totalTransform, cacheEntry.bounds[0],
                          cacheEntry.bounds[1]);
        nvgTransformPoint(&cacheEntry.bounds[2], &cacheEntry.bounds[3], totalTransform, cacheEntry.bounds[2],
                          cacheEntry.bounds[3]);

        memcpy(cacheEntry.currentTransform, state->xform, 6 * sizeof(float));

        nvg__renderFill(ctx->backend, &fillPaint, state->compositeOperation, &state->scissor, ctx->fringeWidth,
                        cacheEntry.bounds, cacheEntry.paths.data(), (int)cacheEntry.paths.size());
        return 1;
    }
...
void nvgStroke(NVGcontext *ctx) {
    ...
    if (state->shapeAntiAlias && (state->lineStyle <= 1 || state->lineStyle == 5)) {
        nvg__expandStroke(ctx, strokeWidth * 0.5f, ctx->fringeWidth, state->lineCap, state->lineJoin, state->lineStyle,
                          state->miterLimit);
    } else {
        nvg__expandStroke(ctx, strokeWidth * 0.5f, 0.0f, state->lineCap, state->lineJoin, state->lineStyle,
                          state->miterLimit);
    }
...
static void nvg__renderTrianglesSimple(NVGcontext *ctx, const NVGvertex *verts, int nverts, NVGpaint fillPaint) {
    ...
    nvg__renderTriangles(ctx->backend, &fillPaint, state->compositeOperation, &scissor, verts, nverts, ctx->fringeWidth,
                         0);
...
void nvgFillRect(NVGcontext *ctx, float x1, float y1, float w, float h) {
    ...
    NVGvertex verts[] = {{x1, y1, t[0], t[1]}, {x2, y2, t[2], t[3]}, {x2, y1, t[2], t[1]},

                         {x1, y1, t[0], t[1]}, {x1, y2, t[0], t[3]}, {x2, y2, t[2], t[3]}};
...
void nvgStrokeRect(NVGcontext *ctx, float x1, float y1, float w, float h) {
    ...
    const int16_t iT[] = {static_cast<int16_t>(t[0] + sx * (1 << 14)), static_cast<int16_t>(t[1] + sy * (1 << 14)),
                          static_cast<int16_t>(t[2] - sx * (1 << 14)), static_cast<int16_t>(t[3] - sy * (1 << 14))};
...
void nvgDrawRoundedRect(NVGcontext *ctx, float x, float y, float w, float h, NVGcolor icol, NVGcolor ocol,
                        float radius) {
...
void nvgDrawObjectWithFlag(NVGcontext *ctx, float x, float y, float w, float h, NVGcolor icol, NVGcolor ocol,
                           NVGcolor flagCol, float radius, ObjectFlagType flagType, bool flagOutline) {
    float shortestSide = std::min(w, h);
...
void nvgFillRoundedRect(NVGcontext *ctx, float x, float y, float w, float h, float radius) {
    nvgDrawRoundedRect(ctx, x, y, w, h, nvg__getState(ctx)->fill.innerColor, nvg__getState(ctx)->fill.innerColor,
                       radius);
}
...
static int nvg__allocTextAtlas(NVGcontext *ctx) {
    ...
    if (ctx->fontImages[ctx->fontImageIdx + 1] != 0) {
        ...
        ctx->fontImages[ctx->fontImageIdx + 1] =
            nvg__renderCreateTexture(ctx->backend, NVG_TEXTURE_ALPHA, iw, ih, 0, NULL);
    }
    ++ctx->fontImageIdx;
...
static void nvg__renderText(NVGcontext *ctx, NVGvertex *verts, int nverts) {
    ...
    nvg__renderTriangles(ctx->backend, &paint, state->compositeOperation, &state->scissor, verts, nverts,
                         ctx->fringeWidth, 1);
...
void nvgTextBox(NVGcontext *ctx, float x, float y, float breakRowWidth, const char *string, const char *end) {
    ...
    int valign = state->textAlign &
                 (NVG_ALIGN_TOP | NVG_ALIGN_MIDDLE | NVG_ALIGN_MIDDLE_ASCENT | NVG_ALIGN_BOTTOM | NVG_ALIGN_BASELINE);
    float lineh = 0;
